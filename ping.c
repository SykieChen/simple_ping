...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hvc:t:bq")) != -1)
	{
		switch (c)
		{
		case 'v':
			...
			break;
		case 'h':
			printf("usage: ping [-hvctbq] <hostname>\n");
			printf("Options:\n");
			printf("	-h		Display help information.\n");
			printf("	-v		Verbose mode, echo not only ICMP_ECHOREPLY message, but also all returned package.\n");
			printf("	-c count	Number of echo requests to send.\n");
			printf("	-t ttl		Set Time To Live for outgoing ICMP package.\n");
			printf("	-b		Send broadcast ICMP request.\n");
			printf("	-q		Quiet output. Only summary lines at startup and completion.\n");
			exit(0);
		case 'c':
			count = atoi(optarg);
			break;
...
void readloop(int count, int ttl)
{
	int size;
	...
	{
		int bbroadcast = 1;
		setsockopt(sockfd, SOL_SOCKET, SO_BROADCAST, (char *)&bbroadcast, sizeof(int));
	}

	sig_alrm(SIGALRM); /* send first packet */
	for (int i = 0; i < (count > 0 ? count : 100); i += (count > 0 ? 1 : 0))
	{
		len = pr->salen;
...
void show_stat()
{
	...
	int utime_used = (e_time.tv_sec-s_time.tv_sec) * 1000000 + e_time.tv_usec-s_time.tv_usec;
	printf("\n--- %s ping statistics ---\n", host);
	printf("%d packets transmitted, %d received, %d%% packet loss, time %dms\n",
		   nsent, nrecv, (nsent-nrecv>0)?100*(nsent-nrecv)/nsent:0, utime_used/1000);
	return;
}
